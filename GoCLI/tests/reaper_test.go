...
// TestDependencyUnblocking verifies that when a blocking task is finished,
// its dependent task transitions from blocked to available.
func TestDependencyUnblocking(t *testing.T) {
	...
	task, err := store.Claim(db, "agent-worker")
	if err != nil {
	...
	// Finish the parent — should mark it finished and unblock the child
